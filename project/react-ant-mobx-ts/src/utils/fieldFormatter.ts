 ...
 */
export function decimalizeMoney(n:string|number, digit=2) {
    let _n = +n
 ...
 * 返回string
 */
export function formatDate(d:any, f='YYYY-MM-DD') {
    if (!d)
        return null
 ...
 * format对象中所有moment值
 */
export function mapMoment(obj:object, f='YYYY-MM-DD') {
    if (!obj)
        return null
...
export function disablePickerDate(type='day', beforeMonth=moment(), afterMonth=moment()) {
    return (current) => {
        ...
        let before = moment(beforeMonth).startOf(type as any)
        let after  = moment(afterMonth).endOf(type as any)
