...
function createReducers(...items) {
    ...
    function _k2action(k: string): string {
 ...
 * 
 * 这些reducers都以唯一key的形式export
 * 最后被如下方式combine到container
 * combineContainer(ListContainer).withReducers(reducers)
 * 
 * 业务代码中(比如XFormContainer)
 * 可以通过 `this.props.formData` 和 `this.props.tableData`
 * 的方式引用这些对象
 * 
 * @param {String} formActionStr
 * @param {String} tableActionStr
 * @returns Reducer
 */
function createListPageReducer(
    formActionStr: string,
    tableActionStr: string,
    ): Reducer4List {
    return function (previousState = defaultState, action) {
